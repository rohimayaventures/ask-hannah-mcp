...
| `hannah_generate_resume` | Tailored resume as **Zod-validated JSON** from the model, then **Phase 3** fact verification on model-owned fields against **hannah-data**, then **deterministic Markdown**; verified name and contact lines come from profile data, not the model; long postings may use Phase 1 JOB SIGNALS extraction; `structuredContent.documentJson` on success; `ERR_RESUME_FACT_DRIFT` when verification fails |
| `hannah_generate_cover_letter` | Same pipeline: validated JSON, Phase 3 verification, deterministic Markdown; optional JD extraction; `documentJson` on success; `ERR_COVER_LETTER_FACT_DRIFT` when verification fails |
...
- **Phases 4–5 (next):** ATS-oriented modes and optional export or handoff to the portfolio resume builder. The repository README tracks the full sequence as each phase lands.
...
| AI generation | Anthropic Messages API (models via env; Sonnet-class defaults for documents, Haiku-class default for JD JSON extract) | Resume and cover letter: validated **document JSON**, **Phase 3** corpus-backed fact checks on model-owned text, then server-side Markdown; optional JOB SIGNALS pre-pass; profile-owned resume header; `documentJson` on success; schema, JSON, and fact-drift error codes; retries; `jd_extract` + `generation` telemetry (no posting body in logs) |
...
| Resume generation | Working | Model returns JSON validated with Zod; Phase 3 fact verification; Markdown rendered deterministically; contact block from profile; long JDs may use JOB SIGNALS extraction first; failures return ERR_RESUME_JSON / ERR_RESUME_SCHEMA / ERR_RESUME_FACT_DRIFT |
| Cover letter generation | Working | JSON with three paragraphs validated; Phase 3 verification; same JD pipeline as resume; ERR_COVER_LETTER_JSON / ERR_COVER_LETTER_SCHEMA / ERR_COVER_LETTER_FACT_DRIFT on failure |
